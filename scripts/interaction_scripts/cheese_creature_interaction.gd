...
func _input(event):
	if !Inventory.cheese_has_interacted:
		Inventory.items_disabled_checker()
	if interaction_times == 0 and interact and !Inventory.cheese_has_interacted and event.is_action_pressed("interact") and message_displayed and interact and Input.is_action_just_pressed("interact") and Transition.canvas_layer.visible == false and Inventory.inventory_layer.visible == false and !Inventory.all_items_enabled:
		...
		index += 1
		show_message()
	if interaction_times == 1 and interact and !Inventory.cheese_has_interacted:
...
func item_give_structure():
	if Inventory.items[Inventory.index].disabled and !Inventory.cheese_has_interacted:
		Inventory.items[Inventory.index].texture_normal = Inventory.item_texture[8]
		Inventory.items[Inventory.index].texture_hover = Inventory.item_texture_hover[8]
		Inventory.items[Inventory.index].disabled = false
		Inventory.cheese_has_interacted = true
